...
export default function EmailPrompt({ onClose, message }: EmailPromptProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="rounded-2xl p-8 max-w-sm w-full shadow-xl"
        style={{ backgroundColor: 'var(--bg-card)', border: '0.5px solid var(--border)' }}
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-xl font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>Sign in to continue</h2>
        ...
        <div className="flex flex-col gap-3">
          <button
            ...
            className="w-full py-3 px-4 rounded-xl font-medium flex items-center justify-center gap-3 transition-opacity disabled:opacity-50"
        ...
        {error && <p className="text-xs mt-3" style={{ color: '#ED93B1' }}>{error}</p>}
